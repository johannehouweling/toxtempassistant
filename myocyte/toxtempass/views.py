...
def legal_document(request: HttpRequest, document: str) -> HttpResponse:
    """Render the terms of service or the license as a page, or as a modal fragment.

    Modals fetch ``?partial=1`` when they open, so the text isn't embedded in every
    page. The HTML is generated by pandoc when the Docker image is built; outside
    the image it doesn't exist and a short note is shown instead.
    """
    title, template_name = config._legal_documents[document]
    try:
        body = render_to_string(template_name)
    except TemplateDoesNotExist:
        body = format_html(
            "<p>The {} is generated from Markdown by pandoc when the Docker image is"
            " built (see md_content/dependencies/run_pandoc.py).</p>",
            title,
        )
    if request.GET.get("partial"):
        ...
        return response
    return render(request, "legal_document.html", {"title": title, "body": body})


@require_GET
def about(request: HttpRequest) -> HttpResponse:
    """Render the public about page, with its FAQ also as FAQPage structured data."""
    ...
    context = {
        "faq": config._about_faq,
        # Constant text with <, > and & escaped, so it can't close the <script>.
        "faq_jsonld": mark_safe(faq_jsonld),  # noqa: S308
    }
    return render(request, "about.html", context)


@require_GET
def toxtemp_questions(request: HttpRequest) -> HttpResponse:
    """Render the public ToxTemp page: every question with the template's notes.

    The template and its notes are by Krebs et al. (2019); the page credits them first.
    """
...
def ror_organization_lookup(request: HttpRequest) -> JsonResponse:
    ...
    items = ror.suggest_organizations(
        request.GET.get("q") or "", request.GET.get("email", "")
    )
...
class AssayListView(SingleTableView):
    ...
    def dispatch(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        """Serve the landing page to visitors and gate users awaiting beta admission.

        Anonymous visitors get the public landing page at "/" itself instead of a
        redirect to /login/, so search engines index the site root. Users who have
        requested beta access but are not yet admitted go to the beta waiting page.
        """
        if not is_logged_in(request.user):
            return render(
                request, "login.html", {"form": LoginForm(), "action": reverse("login")}
            )
