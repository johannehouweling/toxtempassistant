...
* **Account deletion** is refused while the user owns a workspace with other members (never delete those on their behalf; workspaces nobody else is in go with the account), is a superuser, or created a question set. `privacy.delete_account` removes owned investigations first (`Investigation.owner` is PROTECT); the unconfirmed-signup cleanup uses the same function. Users can download every ToxTemp they can open, except the read-only demo (`privacy.export_toxtemps_zip`, JSON and Markdown), before deleting.
